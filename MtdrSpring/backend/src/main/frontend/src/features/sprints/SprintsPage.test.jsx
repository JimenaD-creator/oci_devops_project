/**
 * Requirement 1: Real-time display of tasks assigned to users.
 * Topics: Mock HTTP, React Testing Library, User Events.
 */
...
// Requirement 1: tasks are visible as soon as the page loads 

test('REAL-TIME: tasks assigned to developers appear immediately after load', async () => {
  setupFetchMock([
    {
    ...
    {
      test: (url) => {
        try { return new URL(url, 'http://localhost').pathname === '/api/projects/1'; } catch { return false; }
      },
      handle: () => jsonResponse({ id: 1, name: 'Proyecto Demo' }),
    },
    { test: (url) => pathIncludes(url, '/api/sprints'), handle: () => jsonResponse([activeSprint]) },
    {
      test: (url) => pathIncludes(url, '/api/tasks'),
      handle: () => jsonResponse([
        { id: 10, title: 'Tarea de Ana',   status: 'IN_PROGRESS', priority: 'HIGH',   assignedHours: 5, dueDate: '2026-04-20T00:00:00.000Z', assignedSprint: { id: 1 } },
        { id: 11, title: 'Tarea de Bruno', status: 'TODO',        priority: 'MEDIUM', assignedHours: 3, dueDate: '2026-04-22T00:00:00.000Z', assignedSprint: { id: 1 } },
      ]),
    },
    {
      test: (url, init) =>
