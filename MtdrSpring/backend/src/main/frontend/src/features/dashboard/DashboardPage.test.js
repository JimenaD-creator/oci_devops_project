/**
 * Requirement 5: Role-based dashboard.
 * Topics: Mock modules, Mock HTTP, Testing with contexts (MUI theme), dynamic test data.
 */
...
// ── Mock ALL heavy sub-components that use recharts / framer-motion ───────────
...
const tasksPayload = [
  {
    id: 11, title: 'T1', status: 'DONE', assignedHours: 10,
    dueDate: '2026-04-25T00:00:00.000Z', finishDate: '2026-04-24T00:00:00.000Z',
    assignedSprint: { id: 1 },
  },
  {
    id: 12, title: 'T2', status: 'IN_PROGRESS', assignedHours: 5,
...
function setupMocks() {
  setupFetchMock([
    {
      test: (url) => {
        try { return new URL(url, 'http://localhost').pathname === '/api/projects/1'; }
        catch { return false; }
      },
    ...
    {
      test: (url, init) =>
        pathIncludes(url, '/api/sprints') &&
        String(init?.method || 'GET').toUpperCase() === 'GET',
      handle: () => jsonResponse([sprintA]),
    },
    {
      test: (url, init) =>
        pathIncludes(url, '/api/tasks') &&
        String(init?.method || 'GET').toUpperCase() === 'GET',
...
// ── Requirement 5: dashboard shows project name ───────────────────────────────

test('ROLE-BASED: DashboardPage displays the project name from API', async () => {
  setupMocks();
