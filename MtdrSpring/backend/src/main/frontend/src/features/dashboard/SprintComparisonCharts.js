...
function HoursPlannedVsWorkedTooltip({ active, payload }) {
  ...
  return (
    <Box sx={{ ...tooltipPaper, border: `1px solid ${sprintColor}66` }}>
      ...
      <Typography sx={{ color: HOURS_PLANNED_TASK_FILL, display: 'block', mt: 0.75, fontSize: '0.88rem', fontWeight: 700 }}>
        Planned (TASK assigned hours): {planned.toFixed(1)} h
      </Typography>
      <Typography sx={{ color: HOURS_WORKED_USER_TASK_FILL, display: 'block', mt: 0.35, fontSize: '0.88rem', fontWeight: 700 }}>
        Worked (USER_TASK logged): {worked.toFixed(1)} h
      </Typography>
      <Typography sx={{ color: sprintColor, display: 'block', mt: 0.5, fontSize: '0.84rem', fontWeight: 600, opacity: 0.85 }}>
        Open tasks: {pending}
      </Typography>
    </Box>
  );
...
function SprintHoursCompareBlock({ data }) {
  ...
  return (
    <Box sx={{ width: '100%', minWidth: 0 }}>
      ...
      <ResponsiveContainer width="100%" height={chartH}>
        <BarChart data={data} margin={{ top: 4, right: 10, left: 4, bottom: marginBottom }} barCategoryGap={barCategoryGap}>
          <CartesianGrid strokeDasharray="3 3" stroke={GRID_STROKE} vertical={false} />
          <XAxis
            dataKey="shortLabel"
            tick={renderSprintXTick}
          ...
          />
          <YAxis tick={Y_AXIS_TICK} axisLine={false} width={48} />
          <Tooltip {...RECHARTS_BAR_TOOLTIP_PROPS} shared content={<HoursPlannedVsWorkedTooltip />} />
          <Legend
            verticalAlign="top"
