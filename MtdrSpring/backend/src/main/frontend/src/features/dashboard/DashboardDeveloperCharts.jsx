  ...
  return (
    ...
    >
      {sprintDefs.map((sp) => (
        ...
        >
          <Typography sx={{ ...CHART_LEGEND_ITEM_SX, fontWeight: 800, color: '#37474F' }}>
            {sp.shortLabel}
          </Typography>
          <Box
            sx={{
  ...
  const hWorkloadCompareBase = hasCompareData
    ? Math.max(
        ...
        Math.min(
          ...
          270 +
            compareModel.workloadRows.length * (42 + nSprints * 7) +
  ...
  const hHoursCompareBase = hasCompareData
    ? Math.max(
        ...
        Math.min(
          ...
          290 +
            compareModel.hoursRows.length * (42 + nSprints * 7) +
            WORKLOAD_COMPARE_LEGEND_EXTRA +
